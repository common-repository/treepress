				...
				if(bDiagonalConnections) {
					...
					if(len2){
						...
						while(j < len2) {
							...
							var siXFrom = s.getMyRect().x + (30);
							var siYFrom = s.getMyRect().y + s.getMyRect().height;
							var siYTo = s.getMyRect().y;

							var siYMid = Math.round(0.5 + (siYFrom + siYTo) / 2) + ((j+1) * 2);

							console.log(siYMid)

							drawLine(siXFrom+1, siYFrom+10, siXFrom+1, siYMid+10);
							//drawLine(siXFrom+1, siYMid, iXTo+1, siYMid);
